...
$parentModule = 'DCManagement'
if (-not $ModuleName) { $ModuleName = 'DCManagement.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\DCManagement'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\DCManagement.psd1"
$prereqHash = @{
